...
export const getBanner = (Astro: AstroGlobal, clangs: Clang[]): string => {
    ...
    const languageSwitcherItems = clangs.map((clang) => {
        if (hasSwitcher) {
            if (clang.id === clangId)
                ...
                return `<a href="${clang.url}" data-astro-reload>${clang.code}</a>`;
        }
    });
    ...
    bannerHTML = bannerHTML
        .replace('<cookieBannerLanguageSwitch/>', languageSwitcher)
        .replace('<ToggleButton />', hasSwitcher ? TOGGLE_BUTTON : '');
